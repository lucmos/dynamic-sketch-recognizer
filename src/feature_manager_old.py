...
class FeaturesManager:
    ...
    def _check_saved_pickles(dataset_name):
        for label in TIMED_POINTS_SERIES_TYPE:
            if not os.path.isfile(CachePaths.features(dataset_name, label)):
                return False
        return True
    ...
    def _save_feature(self, dict_to_save, to_csv=True):
        utils.save_dataframes(self.dataset_name, dict_to_save, FEATURE, "Saving features...",
                              to_csv, TIMED_POINTS_SERIES_TYPE, self.data_frames[WORDID_USERID])


if __name__ == '__main__':
    FeaturesManager(DATASET_NAME_0, update_data=True, update_features=True)
